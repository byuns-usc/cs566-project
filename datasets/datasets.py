...
def PET():
    ...
    # Process each split
    process_pet_split(pet_trainval_dataset, class_mapping,
                      os.path.join(base_dir, 'train/images'), 
                      os.path.join(base_dir, 'train/masks'), 'train', train_indices)

    process_pet_split(pet_trainval_dataset, class_mapping,
                      os.path.join(base_dir, 'val/images'), 
                      os.path.join(base_dir, 'val/masks'), 'val', val_indices)

    process_pet_split(pet_trainval_dataset, class_mapping,
                      os.path.join(base_dir, 'test/images'), 
                      os.path.join(base_dir, 'test/masks'), 'test', test_indices)
...
def BRAIN():
    ...
    # Filter out hidden files and other system files
    image_paths_train = sorted([os.path.join(image_dir_train, f) for f in os.listdir(image_dir_train) if f.endswith('.nii.gz') and not f.startswith('._')])
    mask_paths_train = sorted([os.path.join(mask_dir_train, f) for f in os.listdir(mask_dir_train) if f.endswith('.nii.gz') and not f.startswith('._')])
    image_paths_test = sorted([os.path.join(image_dir_test, f) for f in os.listdir(image_dir_test) if f.endswith('.nii.gz') and not f.startswith('._')])
