...
class ImageMaskDataset(Dataset):
    def __init__(self, root_dir, dataset_name, split='train', transform=None, mask_transform=None, mask_suffix="", mask_ext=".png"):
        ...
        if self.mask_dir:
            ...
            self.normalized_mask_labels = {key: [float(val) / 255.0 for val in value] for key, value in self.mask_labels.items()}
...
def create_dataloader(root_dir, dataset_name, split='train', batch_size=8, shuffle=True, transform=None, mask_transform=None, mask_suffix="", mask_ext=".png"):
    dataset = ImageMaskDataset(root_dir, dataset_name, split=split, transform=transform, mask_transform=mask_transform, mask_suffix=mask_suffix, mask_ext=mask_ext)
...
coco_loader_test = create_dataloader(
    coco_root_dir,
    "COCO",
    split='test',
    transform=transform
...
voc_loader_val = create_dataloader(
    voc_root_dir,
    "VOC",
    split='val',
    transform=transform,
...
voc_loader_test = create_dataloader(
    voc_root_dir,
    "VOC",
    split='test',
    transform=transform
...
pets_loader_test = create_dataloader(
    pets_root_dir,
    "PET",
    split='test',
    transform=transform
...
brain_loader_test = create_dataloader(
    brain_root_dir,
    "BRAIN",
    split='test',
    transform=transform
...
heart_loader_test = create_dataloader(
    heart_root_dir,
    "HEART",
    split='test',
    transform=transform
