...
class Bottleneck(nn.Module):
    def __init__(self, scale=2, repeat=3, in_channels=3, out_channels=32):
        ...
        self.blocks = nn.ModuleList([
            Conv1DBlock(in_channels if i==0 else out_channels, out_channels=out_channels, stride=in_channels if i==0 else out_channels)
...
class OneEncoder(nn.Module):
    def __init__(self, opts):
        ...
        for i in range(self.opts['num_layers']):
            ...
            self.convs[f'channel_1_{i}'] = Conv1DBlock(kernel_size=2*self.channels[i+1], out_channels=self.channels[i+1], stride=2*self.channels[i+1])
            self.convs[f'channel_2_{i}'] = Conv1DBlock(kernel_size=self.channels[i+1], out_channels=self.channels[i+1], stride=self.channels[i+1])
